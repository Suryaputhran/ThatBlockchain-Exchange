...
const decorateMyOpenOrder = (order, tokens) => {
    ...
    return ({
        ...order, orderType, orderTypeClass: (orderType === "buy" ? GREEN : RED)
    })
}
...
const decorateFilledOrders = (orders, tokens) => {
    ...
    return (orders.map((order) => {
        ...
        previousOrder = order  // Updates the previous order once it is decorated
        return order
    }))
}
...
const decorateFilledOrder = (order, previousOrder) => {
    return ({
        ...order, tokenPriceClass: tokenPriceClass(order.tokenPrice, order.id, previousOrder)
    })
}
...
export const orderBookSelector = createSelector(openOrders, tokens, (orders, tokens) => {
    ...
    orders = {
        ...orders, buyOrders: buyOrders.sort((a, b) => b.tokenPrice - a.tokenPrice)
    }
    ...
    orders = {
        ...orders, sellOrders: sellOrders.sort((a, b) => b.tokenPrice - a.tokenPrice)
    }
