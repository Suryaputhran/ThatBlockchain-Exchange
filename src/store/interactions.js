...
export const subscribeToEvents = (decentralizedexchange, dispatch) => {
    decentralizedexchange.on("Cancel", (id, user, tokenGet, amountGet, tokenGive, amountGive, timestamp, event) => {
...
export const transferTokens = async (provider, decentralizedexchange, transferType, token, amount, dispatch) => {
    ...
    try {
        ...
        if (transferType === "Deposit") {
            transaction = await token.connect(signer).approve(decentralizedexchange.address, amountToTransfer)
            await transaction.wait()
            transaction = await decentralizedexchange.connect(signer).depositToken(token.address, amountToTransfer)
        } else {
            transaction = await decentralizedexchange.connect(signer).withdrawToken(token.address, amountToTransfer)
        }
        await transaction.wait()
...
// ORDERS (BUY & SELL)
export const makeBuyOrder = async (provider, decentralizedexchange, tokens, order, dispatch) => {
...
// CANCEL ORDER
export const cancelOrder = async (provider, decentralizedexchange, order, dispatch) => {

    dispatch({ type: "ORDER_CANCELLATION_REQUESTED" })

    try {
        const signer = await provider.getSigner()
        const transaction = await decentralizedexchange.connect(signer).cancelOrder(order.id)
        await transaction.wait()
    } catch (error) {
